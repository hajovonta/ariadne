...
(defun start-web-server (&optional graph &key (port 8080))
  ...
  (ht:define-easy-handler (handle-load-graph-api :uri "/api/load-graph") ()
    (setf (ht:content-type*) "application/json")
    (handler-case
        (let* ((params (ht:post-parameters*))
               (file-param (cdr (assoc "file" params :test #'string=))))
          (if file-param
              (let* ((file-path (first file-param))
                     (file-name (second file-param))
                     (name (pathname-name (pathname file-name)))
