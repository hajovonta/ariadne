    ...
    (let ((g (gethash name *web-graphs*)))
      (if g
          ...
          (let ((ht (make-hash-table :test 'equal)))
            ...
            (jzon:stringify ht)))))
  (ht:define-easy-handler (handle-load-graph-api :uri "/api/load-graph") ()
    (setf (ht:content-type*) "application/json")
    (let ((post-data (ht:raw-post-data :force-binary t)))
      (handler-case
          (let* ((boundary (cl-ppcre:scan-to-strings
                            "boundary=(.+)" (ht:header-in* :content-type)))
                 (content-type (ht:header-in* :content-type))
                 ;; Parse multipart
                 (params (ht:post-parameters*))
                 (file-param (assoc "file" params :test #'string=))
                 (file-path (second file-param))
                 (file-name (third file-param)))
            (declare (ignore boundary content-type post-data))
            (if (and file-path file-name)
                (let* ((name (pathname-name (pathname file-name)))
                       (ext (pathname-type (pathname file-name)))
                       (content (uiop:read-file-string file-path))
                       (g (make-graph :name name)))
                  (cond
                    ((member ext '("ttl" "turtle") :test #'string-equal)
                     (import-turtle g content))
                    ((member ext '("nt" "ntriples") :test #'string-equal)
                     (import-ntriples g content))
                    (t (import-turtle g content))) ; default to turtle
                  (explorer-add-graph g name)
                  (setf *web-graph* g)
                  (let ((ht (make-hash-table :test 'equal)))
                    (setf (gethash "name" ht) name)
                    (jzon:stringify ht)))
                (let ((ht (make-hash-table :test 'equal)))
                  (setf (gethash "error" ht) "No file uploaded")
                  (jzon:stringify ht))))
        (error (e)
          (let ((ht (make-hash-table :test 'equal)))
            (setf (gethash "error" ht) (princ-to-string e))
