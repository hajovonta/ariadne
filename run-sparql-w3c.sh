...
for cat in $CATS; do
  ...
  result=$(timeout 120 sbcl --noinform --non-interactive \
    ...
    --eval "(multiple-value-bind (p f e) (run-category \"${cat}\") (format t \"~%RESULT: ~A ~A ~A~%\" p f e))" \
    2>/dev/null | grep "^RESULT:")
  if [ -n "$result" ]; then
    p=$(echo "$result" | awk '{print $2}')
    f=$(echo "$result" | awk '{print $3}')
    e=$(echo "$result" | awk '{print $4}')
    echo "  $p pass, $f fail, $e skip"
    TOTAL_P=$((TOTAL_P + p))
    TOTAL_F=$((TOTAL_F + f))
    TOTAL_E=$((TOTAL_E + e))
  else
...
echo ""
echo "=== TOTAL: $TOTAL_P pass, $TOTAL_F fail, $TOTAL_E skip (of $((TOTAL_P + TOTAL_F + TOTAL_E))) ==="
