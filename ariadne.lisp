...
(defun nested-push (outer-ht outer-key inner-key triple)
  "Push triple into nested hash: outer-ht[outer-key][inner-key] → list."
  (let ((inner (gethash outer-key outer-ht)))
    (unless inner
      (setf inner (make-hash-table :test 'equal))
      (setf (gethash outer-key outer-ht) inner))
    (push triple (gethash inner-key inner))))
  ...
  (let ((key (list subject predicate object)))
    (unless (gethash key (graph-spo g))
      (let ((tr (%make-triple subject predicate object nil)))
        ...
        (nested-push (graph-sp g) subject predicate tr)
        (push tr (gethash subject (graph-s g)))
        (push tr (gethash predicate (graph-p g)))
        (nested-push (graph-po g) predicate object tr)
        (push tr (gethash object (graph-o g)))
        (nested-push (graph-os g) object subject tr)
        (incf (graph-count g))
        tr))))
...
(defun nested-delete (outer-ht outer-key inner-key triple)
  "Remove triple from nested hash. Cleans up empty inner tables."
  (let ((inner (gethash outer-key outer-ht)))
    (when inner
      (setf (gethash inner-key inner) (delete triple (gethash inner-key inner) :test #'eq))
      (when (null (gethash inner-key inner))
        (remhash inner-key inner))
      (when (zerop (hash-table-count inner))
        (remhash outer-key outer-ht)))))
  ...
  (bt:with-lock-held ((graph-lock g))
    (let* ((key (list subject predicate object))
      ...
      (when tr
        ...
        (nested-delete (graph-sp g) subject predicate tr)
        (index-delete (graph-s g) subject tr)
        (index-delete (graph-p g) predicate tr)
        (nested-delete (graph-po g) predicate object tr)
        (index-delete (graph-o g) object tr)
        (nested-delete (graph-os g) object subject tr)
        (decf (graph-count g))
...
(defmethod get-triples ((g graph) &key subject predicate object)
  ...
  (cond
    ...
    ((and subject predicate)
     (let ((inner (gethash subject (graph-sp g))))
       (when inner (copy-list (gethash predicate inner)))))
    ((and predicate object)
     (let ((inner (gethash predicate (graph-po g))))
       (when inner (copy-list (gethash object inner)))))
    ((and object subject)
     (let ((inner (gethash object (graph-os g))))
       (when inner (copy-list (gethash subject inner)))))
