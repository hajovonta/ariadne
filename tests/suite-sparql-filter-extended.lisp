...
(test filter-uri-equality
  ...
  (let ((g (make-graph :name "filt-uri")))
    ...
    (let ((results (sparql g "SELECT ?s ?v WHERE { ?s ?p ?v . FILTER (?p = <http://ex.org/name>) }")))
      (is (= 1 (length results)))
...
(test filter-not-operator
  ...
  (let ((g (make-graph :name "filt-not")))
    ...
    (let ((results (sparql g "SELECT ?s WHERE { ?s <http://ex.org/val> ?v . FILTER (!(?v = 1)) }")))
      (is (= 1 (length results))))))
...
(test filter-bound
  ...
  (let ((g (make-graph :name "filt-bound")))
    ...
    (let ((results (sparql g "SELECT ?s WHERE { ?s <http://ex.org/p> ?o . FILTER (bound(?o)) }")))
      (is (= 1 (length results))))))
...
(test filter-boolean-and
  ...
  (let ((g (make-graph :name "filt-and")))
    ...
    (let ((results (sparql g "SELECT ?s WHERE { ?s <http://ex.org/x> ?v . FILTER (?v > 2 && ?v < 10) }")))
      (is (= 2 (length results))))))
...
(test filter-boolean-or
  ...
  (let ((g (make-graph :name "filt-or")))
    ...
    (let ((results (sparql g "SELECT ?s WHERE { ?s <http://ex.org/x> ?v . FILTER (?v = 1 || ?v = 10) }")))
      (is (= 2 (length results))))))
...
(test filter-true-false
  ...
  (let ((g (make-graph :name "filt-bool")))
    ...
    (is (= 1 (length (sparql g "SELECT ?s WHERE { ?s ?p ?o . FILTER (true) }"))))
    (is (= 0 (length (sparql g "SELECT ?s WHERE { ?s ?p ?o . FILTER (false) }"))))))
