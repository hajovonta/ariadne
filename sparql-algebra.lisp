           ...
           (let ((tmpl (if (and vars (consp (first vars)) (= 3 (length (first vars))))
             ...
             (let ((triples nil))
               (dolist (mu results (remove-duplicates (nreverse triples) :test #'equal))
                 ;; Fresh blank nodes for each result row (for collection templates)
                 (let ((row-bnodes nil))
                   (flet ((resolve-term (term)
                            (let ((v (subst-vars term mu)))
                              (if (and (variable-p v)
                                       (let ((n (symbol-name v)))
                                         (and (> (length n) 6) (string= "?_COLL" (subseq n 0 6)))))
                                  (or (cdr (assoc v row-bnodes))
                                      (let ((bn (format nil "_:coll~A" (incf bnode-counter))))
                                        (push (cons v bn) row-bnodes)
                                        bn))
                                  v))))
                     (dolist (tp tmpl)
                       (let ((s (resolve-term (first tp)))
                             (p (resolve-term (second tp)))
                             (o (resolve-term (third tp))))
